...
export class MapLayer extends Sprite {
     ...
     * @internal
     */
    private _gridSpriteMap = new Map<string, GridSprite>();
            ...
            case "objectgroup":
                ...
                for (var i: number = 0; i < tArray.length; i++) {
                    ...
                    if (tObjectData.visible == true) {
                        ...
                        var tSprite: GridSprite = map.getSprite(tObjectGid, tObjWidth, tObjHeight);
                        ...
                        if (tSprite != null) {
                            ...
                            this._gridSpriteMap.set(tSprite.name, tSprite);
    ...
    getDrawSprite(gridX: number, gridY: number): GridSprite {
        ...
        tSprite.name = key;
        this._gridSpriteMap.set(key, tSprite); // 替换为新的，老对象destroy后无法重用
     ...
     * @param	gridY 当前Grid的Y轴索引
     */
    removeDrawSprite(gridX: number, gridY: number) {
        let key = gridX + "_" + gridY;
        let tSprite = this._gridSpriteMap.get(key);
        tSprite && tSprite.clearAll();
        this._gridSpriteMap.delete(key);
    }
     ...
     */
    updateGridPos(): void {
        this._gridSpriteMap.forEach((tSprite: GridSprite) => {
    ...
    clearAll(): void {
        ...
        this._gridSpriteMap.forEach((tGridSprite: GridSprite) => {
            tGridSprite.clearAll();
        });
        this._gridSpriteMap.clear();
    ...
    addObject(tObjectData: any): GridSprite {
        const t = this;
        if (tObjectData == null || (tObjectData.id == null && tObjectData.name == null)) return null;
        ...
        if (gid) {
            ...
            tSprite.graphics.drawImage(texture, 0, 0, tObjectData.width, tObjectData.height);
        ...
        tSprite.updatePos();
        t.addChild(tSprite);
        t._gridSpriteMap.set(tSprite.name, tSprite);
    ...
    removeObject(tSprite: GridSprite): GridSprite {
        ...
        t._gridSpriteMap.delete(tSprite.name);
        tSprite.removeSelf();
