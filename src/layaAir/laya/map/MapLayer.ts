...
export class MapLayer extends Sprite {
		...
		switch (layerData.type) {
			...
			case "objectgroup":
				...
				for (var i: number = 0; i < tArray.length; i++) {
					...
					if (!tObjectData.name) tObjectData.name = tObjectData.id;
					...
					if (tObjectData.visible == true) {
						...
						tObjectGid = (tObjectData.gid << 4) >>> 4;
						var tSprite: GridSprite = map.getSprite(
							tObjectGid,
							tObjWidth,
							tObjHeight
						);
						let temp = tObjectData.gid >>> 28;
						if (temp & 4) tSprite.scaleY = -1;
						if (temp & 8) tSprite.scaleX = -1;
						if (tSprite != null) {
							switch (this._map.orientation) {
								case IMap.TiledMap.ORIENTATION_ISOMETRIC:
									this.getScreenPositionByTilePos(
										tObjectData.x / tTileH,
										tObjectData.y / tTileH,
										Point.TEMP
									);
									tSprite.pivot(tObjWidth / 2, tObjHeight / 2);
									tSprite.rotation = tObjectData.rotation;
									tSprite.x = tSprite.relativeX =
										Point.TEMP.x + this._map.viewPortX;
									tSprite.y = tSprite.relativeY =
										Point.TEMP.y + this._map.viewPortY - tObjHeight / 2;
									break;
	...
	getScreenPositionByTilePos(
		...
		if (screenPos) {
			switch (this._map.orientation) {
				case IMap.TiledMap.ORIENTATION_ISOMETRIC:
					screenPos.x =
						this._map.width / 2 - (tileY - tileX) * this._tileWidthHalf;
				...
				case IMap.TiledMap.ORIENTATION_STAGGERED:
					...
					screenPos.x =
						tileX * this._map.tileWidth + (tileY & 1) * this._tileWidthHalf;
				...
				case IMap.TiledMap.ORIENTATION_HEXAGONAL:
					...
					screenPos.x =
						(tileX * this._map.tileWidth + (tileY % 2) * this._tileWidthHalf) %
						this._map.gridWidth;
	...
	updateGridPos(): void {
		...
		for (var i: number = 0; i < this._gridSpriteArray.length; i++) {
			...
			if (
				(tSprite.visible || tSprite.isAloneObject) &&
				tSprite.drawImageNum > 0
			) {
				tSprite.updatePos();
			}
		}
	...
	addObject(tObjectData: any): GridSprite {
		const t = this;
		if (
			tObjectData == null ||
			(tObjectData.id == null && tObjectData.name == null)
		)
			return null;
		var tSprite: GridSprite = Pool.createByClass(GridSprite);
		tSprite.name = tObjectData.name ? tObjectData.name : tObjectData.id + "";
		tSprite.initData(this._map);

		tSprite.relativeX = tObjectData.x;
		tSprite.relativeY = tObjectData.y;
		if (tObjectData.rotation) tSprite.rotation = tObjectData.rotation;
		//数据上标记这个对象已经显示到舞台
		tObjectData.visible = true;
		tSprite.visible = true;
		tSprite.zOrder = tObjectData.y;
		let gid = tObjectData.gid;
		if (gid) {
			let tObjectGid = (tObjectData.gid << 4) >>> 4;
			let temp = tObjectData.gid >>> 28;
			if (temp & 4) tSprite.scaleY = -1;
			if (temp & 8) tSprite.scaleX = -1;
			tSprite.pivot(tObjectData.width / 2, tObjectData.height / 2);
			let tTileTexSet: TileTexSet = this._map.getTexture(tObjectGid);
			let texture = tTileTexSet.texture;
			tSprite.graphics.clear();
			tSprite.graphics.drawImage(
				texture,
				0,
				0,
				tObjectData.width,
			...
			tSprite.width = tObjectData.width;
			tSprite.height = tObjectData.height;
		}
		tSprite.drawImageNum++; // 默认都有
		tSprite.updatePos();
		t.addChild(tSprite);
	...
	removeObject(tSprite: GridSprite): GridSprite {
		const t = this;
		if (tSprite == null) return null;
		let name = tSprite.name;
		delete t._objDic[name];
		let tObjectData = t._dataDic[name];
		if (tObjectData) tObjectData.visible = false;
		delete t._dataDic[name];
		let arr = t._gridSpriteArray;
		let index = arr.indexOf(tSprite); // 找到要删除的元素的索引
		if (index > -1) {
		...
		// tSprite.zOrder = 0;
		// tSprite.graphics.clear();
		// tSprite.destroyChildren();
		// tSprite._destroyAllComponent();
		// tSprite.offAll();
		Laya.timer.clearAll(tSprite);
		tSprite.clearAll(); // 这里会distroy,不能调用
		// tSprite["_map"] = null;
		// tSprite.relativeX = 0;
		// tSprite.relativeY = 0;
		// tSprite.isHaveAnimation = false;
		// tSprite.aniSpriteArray = null;
		// tSprite.drawImageNum = 0;
		// Pool.recoverByClass(tSprite); // 回收使用有非常多的问题
		return tSprite;
	}
}
